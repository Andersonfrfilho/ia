...
- **Contrato de Listagens Tabulares:** Todo endpoint que alimenta uma tabela/grid (ver `web.md` seção 7 e `mobile.md` seção 8) deve expor `sortBy`, `sortDirection` (`asc`/`desc`), `filters[]` (aceitando múltiplos valores por campo) e paginação (`page`/`perPage` ou cursor) como parâmetros de entrada documentados, aplicando-os na API antes de devolver o payload — nunca delegando ordenação/filtragem de grandes volumes só ao cliente.
